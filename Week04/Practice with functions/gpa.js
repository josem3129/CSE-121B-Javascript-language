function getGrades(inputSelector) {
    // get grades from the input box
    const grades = document.querySelector("#grades").value;
    // split them into an array (String.split(','))
    const splitedGradeArray = grades.split(",");
    // clean up any extra spaces, and make the grades all uppercase. (Array.map())
    const cleanGrade = splitedGradeArray.map((grade) => grade.trim().toUpperCase());
    console.log(cleanGrade);
    // return grades
    return cleanGrade;
  }
  
  function lookupGrade(grade) {
    // converts the letter grade to it's GPA point value and returns it
    let gpaGrade = 0;
    if (grade === 'A')
    {
        gpaGrade = 4;
    }
    else if (grade === 'B')
    {
        gpaGrade = 3;
    }
    else if (grade === 'C')
    {
        gpaGrade = 2;
    }
    else if (grade === 'D')
    {
        gpaGrade = 1;
    }
    
    return gpaGrade;
   
  }
  
  function calculateGpa(grades) {
    // gets a list of grades passed in
    // convert the letter grades to gpa points
    const gradePoints = grades.map((grade) => lookupGrade(grade));
    // calculates the GPA
    const gpa =
      gradePoints.reduce((total, num) => total + num) / gradePoints.length;
    // return the GPA
    return gpa.toFixed(2);
  }
  
  function outputGpa(gpa, selector) {
    // takes a gpa value and displays it in the HTML in the element identified by the selector
    document.getElementById(selector).innerHTML = gpa;
  }
  
  function clickHandler() {
    // when the button in our html is clicked:
    // get the grades entered into the input
    const grades = getGrades();
    // calculate the gpa from the grades entered
    const calculate = calculateGpa(grades)
    // display the gpa
    outputGpa(calculate, 'output')
}
document.getElementById('submitButton').addEventListener('click', clickHandler)